...
- [GPU 在 Kubernetes 上的调度详解](docs/02-cloud-native/gpu-scheduling/) — 覆盖 NVIDIA Device Plugin gRPC 协议（registration、ListAndWatch、Allocate、PreStartContainer、health check）、GPU Operator 组件与生命周期、`nvidia.com/gpu` 资源模型、MIG/MPS/time-slicing/vGPU 共享、拓扑感知调度（NUMA/PCIe/NVSwitch）、Gang/队列/公平调度（Volcano/scheduler-plugins/Kueue/v1.36 PodGroup）、多租户配额/优先级/抢占、生产排障与可观测、源码分析（k8s-device-plugin/GPU Operator/scheduler-plugins/Volcano/Kueue）、纯 Python 可运行 Mini Demo（44 测试，复现 Device Plugin → kubelet → scheduler → Gang → 拓扑 → 共享全链路）、面试题与最佳实践（内容更新至 2026-07-05）
...
- [RL Post-Training 基础设施详解](docs/04-llmops/rl-post-training/) — 覆盖 o1/R1 后 RL 成为大模型训练主战场：RLHF/RLVR 区别、PPO/GRPO/DPO 算法工程化、可验证奖励 vs Reward Model、Rollout/Train 分离三种拓扑（Co-located/Disaggregated/Hybrid）、权重同步（NCCL/量化/流水线）、长尾治理、Agentic RL 挑战、veRL/OpenRLHF 源码分析、DeepSeek/字节/阿里/Kimi 生产实践、纯 Python Mini Demo（bigram 模型 + GRPO + 权重版本同步 + Continuous Batching，28 测试）、最佳实践与面试题（内容更新至 2026-07-21）
