...
| [Planning](/05-agent/planning/) | 如何分解与重规划任务 | Planner 可以利用 Episodic Memory 中的成功案例来生成更优计划 |
| [Tool Use](/05-agent/tool-use/) | 工具调用与执行 | Memory 存储工具结果摘要与偏好，Tool Use 层负责实时调用与结果格式化 |
| [Agent OS](/05-agent/agent-os/) | 运行时操作系统 | Agent OS 管理进程级 Workspace 与状态；Memory 提供跨进程/跨会话的持久化与检索 |
