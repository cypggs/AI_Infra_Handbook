...
| [GPU 在 Kubernetes 上的调度（本手册主题）](/02-cloud-native/gpu-scheduling/) | 完整覆盖 K8s GPU 调度与治理全链路：Device Plugin、GPU Operator、MIG/MPS、拓扑感知、Gang/队列公平调度 |
...
| **[CNI / CSI 深度](/02-cloud-native/cni-csi/)** | K8s 通过网络/存储插件接口把网络与存储外包给社区与云厂商；CNI/CSI 是 K8s 架构的两大扩展点 | 读完本主题后再读 CNI/CSI，理解 kubelet 如何调用 CNI ADD 与 CSI NodePublish |
| **[GPU 在 Kubernetes 上的调度](/02-cloud-native/gpu-scheduling/)** | K8s 把 GPU 作为一等资源进行发现、调度、共享、拓扑感知与治理；覆盖 Device Plugin、GPU Operator、MIG/MPS、Gang、队列公平调度 | 读完本主题后再读 GPU 调度，理解从 extended resource 到容器可见 GPU 的完整链路 |
