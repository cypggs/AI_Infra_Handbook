...
| [分布式系统基础](/01-foundation/distributed-systems/) | Ray 的 GCS、所有权模型、lineage 重建、actor 故障恢复都是分布式系统理论（复制、一致性、容错）的工程实现。 |
| [存储系统](/01-foundation/storage-systems/) | Ray 的 Plasma 对象存储、spillback 到本地磁盘、数据集读取都依赖底层存储选型和性能。 |
